package me.agent.teamselector.teamselector;
                ...
                if (s.contains("{members}")) {
                    ...
                    for (Player p : TeamManager.getInstance().getMembers(bwt, arena)) {
                        if (hasPlaceholderAPI)
                            lore.add(ChatColor.translateAlternateColorCodes('&', color + PlaceholderAPI.setPlaceholders(p, Config.config.getString(Config.CHOICE_FORMAT_NAME))).replace("{name}", p.getDisplayName()));
                        else
                            lore.add(ChatColor.translateAlternateColorCodes('&', color + Config.config.getString(Config.CHOICE_FORMAT_NAME)).replace("{name}", p.getDisplayName()));
                    }
                } else {
                    lore.add(s);
                }
